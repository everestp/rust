...
fn main() {
   ...
   filter_and_map(v1.clone());
   even_file_with_Some(v1.clone());
   even_filer(v1.clone());
}
...
fn  even_filer(v:Vec<i32>){
    let mut filter_even_vec :Vec<i32> = Vec::new();
     for val in v.iter() {
        if val  % 2==0{

        filter_even_vec.push(*val);
        }
     }
      println!("this is doeb byu even_filter{:?}",filter_even_vec);
}


fn even_file_with_Some(v:Vec<i32>){
     let mut filter_even_vec :Vec<i32> = Vec::new();
     if let Some(val) = v.iter().next() {
        if val  % 2==0{

        filter_even_vec.push(*val);
        }
     }
      println!("this is doeb byu even_filter_Some{:?}",filter_even_vec);
